...
import { ReservaZonaCard, ZonaBanner } from "@/features/zonas/components";
...
export function ZonaDetallesScreen() {
  ...
  const relevantDays = useMemo(() => {
    if (dayFilter === "hoy") return [DAYS[new Date().getDay()].toLowerCase()];
    if (dayFilter === "manana") {
      const tomorrow = new Date();
      tomorrow.setDate(tomorrow.getDate() + 1);
      return [DAYS[tomorrow.getDay()].toLowerCase()];
    }
    return [];
  }, [dayFilter]);
  ...
  const freeHours = FREE_HOURS.map((hour) => {
    ...
    const reservations = zoneReservations.filter((reservation) => {
      const match = reservation.horario.match(
        /(\d{1,2})[:\s]*(\d{2})?.*?(\d{1,2})[:\s]*(\d{2})?/,
      );
      if (!match) return false;
      const from = Number(match[1]) * 60 + Number(match[2] || 0);
      const to = Number(match[3]) * 60 + Number(match[4] || 0);
      const hasNamedDay = DAYS.some((day) =>
        reservation.horario.toLowerCase().includes(day.toLowerCase()),
      );
      const matchesDay =
        !relevantDays.length ||
        !hasNamedDay ||
        relevantDays.some((day) =>
          reservation.horario.toLowerCase().includes(day),
        );
      return matchesDay && start >= from && start < to;
    });
        ...
        <View className="gap-2.5">
          {filtered.map((reservation) => (
            <ReservaZonaCard
              ...
              reserva={reservation}
              esGuardia={esGuardia}
              onPress={() => openPeople(reservation)}
              onMenu={() => setMenuItem(reservation)}
            />
          ))}
          {filtered.length === 0 && (
            <Text className="text-center text-sm text-gray-500 py-8">
